...
import { Animation, presets } from "@/core/player/animated";
import { PresetName } from "@/core/player/animated/presets";
...
const BasicCaption = ({
  ...
  useEffect(() => {
    ...
    const currentFont = fonts.find(
      (font: IFont) => font.postScriptName === fontFamily
    );
    const selectedFont = compactFonts.find(
      (font: ICompactFont) => font.family === currentFont?.family
    );
